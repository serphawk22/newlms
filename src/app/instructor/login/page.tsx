  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.role !== "INSTRUCTOR") {
        setError(
          "This login is for instructors only. Please use the correct login page for your role."
        );
        setLoading(false);
        return;
      }
      ...
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>
        ...
        <motion.div variants={itemVariants} className="mt-6">
          <button
            type="button"
            onClick={() => {
              window.location.href = "/api/auth/google?role=INSTRUCTOR&intent=login";
            }}
        ...
        <form onSubmit={handleLogin} className="mt-7 space-y-4">
          ...
          <motion.div variants={itemVariants} className="space-y-1.5">
            <Label htmlFor="instructor-loginCode">Login code</Label>
            <div className="relative">
        ...
        <motion.p variants={itemVariants} className="mt-3 text-center text-sm">
          <Link href="/" className="hover:underline" style={{ color: "var(--muted-foreground)" }}>
            Back to home
          </Link>
        </motion.p>
      </motion.div>
