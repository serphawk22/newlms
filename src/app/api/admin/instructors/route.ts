...
/* GET /api/admin/instructors?orgId= */
export async function GET(req: NextRequest) {
  ...
  try {
        ...
        include: {
          user: {
            select: {
              ...
              coursesCreated: { where: { organizationId: orgId }, select: { id: true } },
            },
          },
        },
        orderBy: { id: "asc" },
    ...
    const instructors = members.map((m) => ({
      memberId: m.id, userId: m.user.id, name: m.user.name || "Unnamed",
      email: m.user.email, coursesCreated: m.user.coursesCreated.length,
    }));
    return NextResponse.json({ instructors, total, page, limit, pages: Math.ceil(total / limit) }, { headers: { "Cache-Control": "public, max-age=30, stale-while-revalidate=60" } });
  } catch (err) {
    console.error("[GET /api/admin/instructors]", err);
    return NextResponse.json({ error: "Failed to fetch instructors" }, { status: 500, headers: { "Cache-Control": "no-cache" } });
  }
}

/* POST /api/admin/instructors  body: { name, email, orgId } */
...
/* DELETE /api/admin/instructors  body: { memberId, orgId } */
export async function DELETE(req: NextRequest) {
  ...
  try {
    const member = await prisma.organizationMember.findFirst({
      where: { id: memberId, organizationId: admin.orgId, role: "INSTRUCTOR" },
    });
