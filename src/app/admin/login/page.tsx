      ...
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>
        ...
        <form onSubmit={handleLogin} className="mt-7 space-y-4">
          ...
          <motion.div variants={itemVariants} className="space-y-1.5">
            ...
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 transition-colors duration-200" style={{ color: focusedField === "password" ? "var(--foreground)" : "var(--muted-foreground)" }} />
              <Input
                id="admin-password"
                name="password"
        ...
        {!showOtherRoles ? (
          ...
          <motion.div variants={itemVariants} className="mt-6 text-center space-y-3">
            <p className="text-xs font-medium" style={{ color: "var(--muted-foreground)" }}>Select your role to sign in:</p>
            <div className="flex justify-center gap-4">
              <Link
                href="/student/login"
                className="px-4 py-2 text-xs font-semibold rounded-lg transition-all duration-200"
                style={{ color: "var(--foreground)", background: "var(--card)", border: "1px solid var(--border)" }}
              >
                Student Login
              </Link>
              <Link
