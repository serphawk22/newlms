...
interface JoinMcqPdfImporterProps {
  courseId: string;
  existingQuestions: ParsedQuestion[];
}

export function JoinMcqPdfImporter({ courseId, existingQuestions }: JoinMcqPdfImporterProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/courses/save-join-mcqs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ courseId, questions: allQuestions }),
      });
      const data = await res.json();
    ...
    <div className="rounded-xl border border-dashed border-[#D9252A] bg-[rgba(217,37,42,0.04)] overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-[#D9252A]">Import Screening Questions from PDF</p>
          <p className="text-xs text-[#D9252A]">Upload a PDF containing course requirements or questions.</p>
        </div>
        {collapsed
        ...
        <div className="px-4 pb-4 space-y-4">
          ...
          {stage === "parsing" && (
            <div className="flex flex-col items-center gap-3 py-6">
              ...
              <p className="text-sm font-medium text-slate-600">Analyzing &ldquo;{fileName}&rdquo;…</p>
              <p className="text-xs text-slate-400">Extracting and generating MCQs with OpenAI</p>
            </div>
          )}
          ...
          {stage === "preview" && (
            <div className="space-y-3">
              ...
              >
                ...
                Add All {questions.length} Questions to Course
              </Button>
            </div>
          )}
